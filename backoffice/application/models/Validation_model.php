...
//        $this->db->where('view_status', 'yes');
    ...
    public function getAllUserData($limit,$offset,$user_id){
        ...
        WHEN learning_system = 'yes' THEN 'learning'
        WHEN forex_system = 'yes' THEN 'forex'
        ELSE 'NA'
    ...
    public function getPackageSupport($user_id,$type){
        $product_id=$this->getProductId($user_id);
        if(in_array($type,array('board','services','tourism','forex','learning'))){
            $field=$type."_system";
    ...
    public function getSponsorNameBasedOnBoardPackage($sponsorname){
        if($sponsorname){
            $user_id=$this->validation_model->userNameToID($sponsorname);
            $status=$this->getPackageSupportAPI($sponsorname,'board');
            if($status=='yes'){
                return $sponsorname;
            }else{
                $sponsor_id=$this->getSponsorId($user_id);
                $sponsorname=$this->idToUsername($sponsor_id);
                return $this->getSponsorNameBasedOnBoardPackage($sponsorname);
            }
            //return 'admin';

        }   
    }
    public function getPackageSupportAPI($sponsorname){

        $curl = curl_init();

        curl_setopt_array($curl, array(
          CURLOPT_URL => BOARD_URL.'/user/check',
          ...
          CURLOPT_POSTFIELDS => array('api_key' => BOARD_API_KEY,'sponsor_username' => $sponsorname),
